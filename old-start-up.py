...
class newProjectWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        self.submit.clicked.connect(lambda: ({""}, self.close()))
        self.testevent = QtGui.QFocusEvent()
        self.projectLocation.textbox.focusInEvent(self.testevent)
...
if __name__ == "__main__":
    ...
    win = MainWindow()
    win.statusBar().showMessage('Made by Lempac')
    closeEvent = QtGui.QCloseEvent()
    closeEvent.spontaneous
    # self, event:( QtGui.QCloseEvent:
    #         close = QtWidgets.QMessageBox(QtGui.QMessageBox.question(self, 'Message',
    #         "Are you sure to quit?", QtGui.QMessageBox.Yes, QtGui.QMessageBox.No)).exec()
